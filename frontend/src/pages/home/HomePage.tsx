...
export function HomePage() {
  const [filters, setFilters] = useState<DepositFiltersState>(initialFilters);

  const [sort, setSort] = useState<DepositSortConfig>(initialSort);
  ...
  const filteredDeposits = useMemo(() => {
    const filtered = filterDeposits(deposits, filters);

    return sortDeposits(filtered, sort);
  }, [deposits, filters, sort]);

  const selectedDeposit = filteredDeposits.find(
      ...
      <section className="hero">
        <div>
          ...
          <p className="hero-description">
            Смотрим не только на рекламную ставку, но и на фактический доход,
            условия и влияние инфляции.
          </p>
        </div>
      ...
      <section className="products">
        <div className="section-header">
          <div>
            ...
            <p>Найдено: {filteredDeposits.length}</p>
          </div>
        </div>
        ...
        {filteredDeposits.length > 0 ? (
          <DepositTable
            ...
            onSelectDeposit={(deposit) => setSelectedDepositId(deposit.id)}
          />
        ) : (
